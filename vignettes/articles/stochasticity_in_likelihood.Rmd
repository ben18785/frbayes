---
title: "Stochasticity in the likelihood"
---
...
A simple model for $F(\text{prey};\theta)$ is:
...
To illustrate how this model works, we can simulate from it in `frbayes` which, under the hood, uses the [Gillespie algorithm](https://arxiv.org/abs/0704.1908). We assume an arbitrary parameter value: $\text{rate}=0.1$ and perform a simulation where we suppose there are initially 10 prey.
```{r}
...
$$
By simply substituting $\text{prey} = n - n_e$, where $n_e$ is the number of prey eaten, we obtain a distribution over the number of prey eaten:
