...
We now fit a model to these data using maximum likelihood estimation. To do so, we use the `log_probability` function. We first show how the log-likelihood varies as $a$ is varied with $h$ fixed at its true value. The log-likelihood is a little rough ([owing to the stochasticity in how we calculate it](https://ben18785.github.io/frbayes/articles/stochasticity_in_likelihood.html)), but it is peaked near the value used to generate the data.
```{r}
as <- seq(1, 4, 0.1)
